...
async function extractYouTubeVideo(url) {
    ...
    try {
        // Use server-side API to extract direct URLs
        const response = await fetch(`/api/youtube-extract?id=${videoId}`);
        if (!response.ok) {
            throw new Error(`YouTube extraction failed: ${response.status}`);
        }
        const data = await response.json();
        // Return direct video URLs in different qualities
        return {
            mp4: data.formats?.filter((format)=>format.container === 'mp4' && format.hasVideo && format.hasAudio).map((format)=>({
                    url: format.url,
...
async function extractVimeoVideo(url) {
    ...
    try {
        // Use server-side API to extract direct URLs
        const response = await fetch(`/api/vimeo-extract?id=${videoId}`);
        if (!response.ok) {
            throw new Error(`Vimeo extraction failed: ${response.status}`);
        }
        const data = await response.json();
        // Return direct video URLs
        return {
            mp4: data.files?.progressive?.map((file)=>({
                    url: file.url,
...
"[project]/src/components/FullPlayer.tsx [app-client] (ecmascript)": (function(__turbopack_context__) {

var { g: global, __dirname, k: __turbopack_refresh__, m: module, e: exports } = __turbopack_context__;
{
const e = new Error(`Could not parse module '[project]/src/components/FullPlayer.tsx'
...
//# sourceMappingURL=_b71dfd62._.js.map